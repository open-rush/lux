...
| 层级 | 文件 | 内容 |
|------|------|------|
| **全局** | `AGENTS.md` | 架构、流程、约定——整个 repo 的指南 |
| **方向** | `specs/*.md` | 设计决策——状态机、协议、架构约束（不重复 README 的细节） |
| **模块** | 各模块 `README.md` | 模块详情——API、子模块结构、用法、依赖、内部设计 |

三者不重叠：AGENTS.md 说"全局怎么协作"，specs 说"方向上我们选了什么"，README 说"这个模块具体怎么用"。

每个自成体系的部分都应有 README。新增模块时同步写 README。
...
每个 app 和 package 有自己的 `README.md`，描述模块职责、子模块结构、API、用法。详见各模块 README。
