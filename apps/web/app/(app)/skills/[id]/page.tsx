    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/skills" className="inline-flex items-center gap-1.5 text-sm text-primary hover:underline">
    ...
    <div className="flex-1 overflow-y-auto">
      <div className="mx-auto max-w-[1400px] px-8 py-10 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {skill.latestVersion && (
            <Badge variant="secondary" className="font-normal text-xs">{skill.latestVersion.replace(/^v/, '')}</Badge>
          )}
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            <div className="relative">
              <button
                type="button"
                onClick={() => handleToggleVisibility(skill.visibility === 'public' ? 'private' : 'public')}
            ...
            {/* Star */}
            <StarButton skillName={skill.name} initialCount={skill.starCount} initialStarred={skill.isStarred} size="md" />

            {/* More actions dropdown */}
            <div className="relative">
              <button type="button" onClick={() => setShowMenu(!showMenu)} className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-accent transition-colors">
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-10 z-50 w-48 rounded-lg border border-border bg-background py-1 shadow-lg">
                    <button type="button" onClick={() => { setShowMenu(false); /* TODO: member dialog */ }} className="flex w-full items-center gap-2 px-3 py-2 text-sm hover:bg-accent">
                      <UsersIcon className="h-3.5 w-3.5" /> 成员管理
                    </button>
                    <div className="my-1 border-t border-border" />
                    <button type="button" onClick={() => { setShowMenu(false); setShowDeleteConfirm(true); }} className="flex w-full items-center gap-2 px-3 py-2 text-sm text-destructive hover:bg-destructive/10">
          ...
          <div className="w-full lg:w-[320px] shrink-0 space-y-6">
            ...
            <div className="rounded-lg border bg-muted/30 p-3">
              <div className="flex items-center justify-between gap-2">
                <code className="font-mono text-sm text-foreground overflow-x-auto flex-1 whitespace-nowrap">{installCmd}</code>
                <button type="button" onClick={handleCopy} className="shrink-0 flex h-7 w-7 items-center justify-center rounded-md hover:bg-background hover:shadow-sm transition-all">
            ...
            <div className="rounded-lg border bg-background p-4 space-y-4">
              ...
              {skill.sourceUrl && (
                <MetadataItem label="Repository">
                  <a href={skill.sourceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 text-primary hover:underline font-medium text-sm">
              ...
              <MetadataItem label="Created">
                <span className="font-medium text-sm">{new Date(skill.createdAt).toLocaleDateString()}</span>
              </MetadataItem>
              ...
              {skill.tags.length > 0 && (
                <MetadataItem label="Tags">
                  <div className="flex flex-wrap gap-1.5">
                    {skill.tags.map((tag) => (
                      <Badge key={tag} variant="secondary" className="px-2 py-0.5 text-xs font-normal">{tag}</Badge>
                    ))}
                  </div>
                </MetadataItem>
              )}
        ...
        {showDeleteConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowDeleteConfirm(false)}>
            <div className="w-full max-w-sm rounded-xl bg-background p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
              ...
              <p className="mt-2 text-sm text-muted-foreground">确定要删除 &ldquo;{skill.name}&rdquo; 吗？</p>
              <p className="mt-1 text-xs text-destructive">此操作不可恢复，所有版本和收藏数据都将被永久删除。</p>
              <div className="mt-4 flex justify-end gap-3">
                ...
                <button type="button" onClick={() => void handleDelete()} disabled={deleting} className="inline-flex items-center gap-2 rounded-lg bg-destructive px-4 py-2 text-sm font-medium text-destructive-foreground hover:bg-destructive/90 disabled:opacity-50">
