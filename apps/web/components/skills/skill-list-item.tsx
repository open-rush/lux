  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-muted-foreground line-clamp-1">{skill.description || skill.source}</p>
      </div>
      ...
      {skill.tags && skill.tags.length > 0 && (
        <div className="hidden lg:flex items-center gap-1.5 shrink-0">
          ...
          {skill.tags.length > 2 && (
            <span className="inline-flex items-center rounded-md bg-secondary px-2 py-0.5 text-xs font-medium">+{skill.tags.length - 2}</span>
          )}
        </div>
      )}
      ...
      <div className="hidden md:flex items-center gap-2 shrink-0 p-1.5 rounded-md bg-muted/50 border border-border">
        <code className="text-xs text-muted-foreground font-mono truncate max-w-[200px]">{installCmd}</code>
        <button type="button" onClick={handleCopy} className="shrink-0 rounded p-0.5 text-muted-foreground hover:text-foreground transition-colors">
