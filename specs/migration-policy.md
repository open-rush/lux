...
2. Generate: pnpm --filter @open-rush/db db:generate (auto-builds before generating)
3. Review the generated SQL in packages/db/drizzle/
4. Test: pnpm --filter @open-rush/db test (PGlite) + test:integration (Docker)
...
DATABASE_URL=postgresql://rush:rush@localhost:5432/rush pnpm --filter @open-rush/db db:migrate
```
